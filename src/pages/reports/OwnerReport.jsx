...
import { getOwnerReport } from '../../api/ReportApi';
...
const OwnerReport = () => {
  ...
  useEffect(() => {
    const fetchOwnerReport = async () => {
      try {
        if (id && token) {
          const response = await getOwnerReport(id, token);
  ...
  const downloadCSV = () => {
    const csvRows = [
      ["Date", "Total Price"],
      ...filteredData.map(row => [row.date, row.totalPrice])
    ];
    const csvContent = csvRows.map(e => e.join(",")).join("\n");
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    saveAs(blob, "owner_report.csv");
  };
